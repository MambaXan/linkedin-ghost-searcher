...
const App: React.FC = () => {
  ...
  const handleClassicSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Исправляем ошибку типизации при обновлении профиля
      if (typeof data.current_usage === "number") {
        const newUsage = data.current_usage;
        setProfile((p) => ({ ...p, search_count: newUsage }));
      }
    } catch (err: any) {
        ...
        <main className="main">
          ...
          {!isSmartMode ? (
            <form className="search-form" onSubmit={handleClassicSubmit}>
              {/* Добавляем плашку paywall, если лимит исчерпан */}
              {isLimited && (
              ...
              <input
                ...
                disabled={isLimited} // Блокируем ввод
                onChange={(e) =>
            ...
            /* Тут твой код для AI Strategist */
