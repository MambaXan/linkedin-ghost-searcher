...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    else setProfile({ search_count: 0, is_pro: false, plan_type: "free" });
  }, [user, fetchProfile]);
          ...
          {!isSmartMode ? (
            <form className="search-form" onSubmit={handleClassicSubmit}>
              ...
              <input className="field" type="text" placeholder="Job Title"
                value={formData.job_title} disabled={isLimited}
              ...
              <input className="field" type="text" placeholder="Company (optional)"
                value={formData.company} disabled={isLimited}
                onChange={e => setFormData({ ...formData, company: e.target.value })} />
              <input className="field" type="text" placeholder="Location (optional)"
                value={formData.location} disabled={isLimited}
                onChange={e => setFormData({ ...formData, location: e.target.value })} />
              <button type="submit"
                className={`btn btn--full${isLimited ? " btn--locked" : " btn--primary"}`}
                disabled={loading || isLimited}>
                {isLimited ? "🔒 Limit Reached" : loading ? "Generating…" : "Generate Search URL"}
              </button>
            </form>
          ) : (
            /* AI Strategist form */
            <form className="search-form" onSubmit={handleAiSubmit}>
              ...
              <input className="field" type="text"
                ...
                value={aiPrompt} disabled={isLimited}
                onChange={e => setAiPrompt(e.target.value)} required />
              <button type="submit"
                className={`btn btn--full${isLimited ? " btn--locked" : " btn--primary"}`}
                disabled={loading || isLimited}>
