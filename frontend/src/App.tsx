...
// // LemonSqueezy Overlay SDK — injected via <script> tag in index.html
// declare global {
//   interface Window {
//     LemonSqueezy: {
...
// // LemonSqueezy checkout URL — embed=1 enables the overlay
...
// }) => {
//   const handleUpgrade = () => {
//     // Must be logged in first so LemonSqueezy can link the purchase to the account
//     if (!isLoggedIn) {
//       onSignIn();
//       return;
//     }

//     // Use LemonSqueezy overlay when SDK is loaded, fall back to new tab
...
//     // 🔥 если уже есть канал — не создаем новый
...
//     // FREE → только подсказка, без магии
//     if (!isPro) {
//       // вытаскиваем только роль (упрощенно)
//       const cleaned = query.split(" at ")[0];

//       setFormData({
//         job_title: cleaned, // 👈 только роль
//         company: "",
//         location: "",
//       });
...
//       else alert("Failed to generate URL. Try again.");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//     // 🔒 2. ГЛАВНОЕ: проверка PRO
//     if (!isPro) {
//       setView("pricing");
//       return;
//     }

//     // 🔒 3. лимит (на всякий случай)
//     if (isLimited) {
//       setView("pricing");
//       return;
//     }
...
//       if (newCount)
//         setProfile((p) => ({ ...p, search_count: Number(newCount) }));
...
//                 }`}
//                 disabled={!isPro || loading || isLimited}
//               >
//                 {!isPro
//                   ? "🔒 PRO only"
//                   : isLimited
...
//                     : "🔒 Upgrade to PRO to use AI Strategist"
//                 }
//                 value={aiPrompt}
    ...
    <div className="app">
      ...
      {view === "search" && (
        <main className="main">
          ...
          {/* Classic form — работает для всех (лимит 5/день для free) */}
          {!isSmartMode ? (
            <form className="search-form" onSubmit={handleClassicSubmit}>
              {isLimited && (
                <div className="search-form__paywall">
